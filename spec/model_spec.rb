...
describe Praha::Model do
  ...
  describe ".adapter" do
    it "sets a new instance of the passed adapter as @adapter" do
      klass = Class.new(Praha::Model) do
        adapter Praha::Adapter
      end

      klass.instance_variable_get(:@adapter).should be_kind_of(Praha::Adapter)
    end

    it "returns the adapter set on the model subclass" do
      klass = Class.new(Praha::Model) { adapter Praha::Adapter }
      klass.adapter.should be_kind_of(Praha::Adapter)
    end

    it "raises an error when no adapter set on model subclass" do
      klass = Class.new(Praha::Model)
